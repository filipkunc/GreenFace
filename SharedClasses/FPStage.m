...
+ (void)initStages
{
	...
	currentStage = 0;
	currentLevelCustom = (int)[[NSUserDefaults standardUserDefaults] integerForKey:@"currentLevelCustom"];
	currentLevelOriginal = (int)[[NSUserDefaults standardUserDefaults] integerForKey:@"currentLevel"];
	maxAvailableLevel = (int)[[NSUserDefaults standardUserDefaults] integerForKey:@"maxAvailableLevel"];
}
...
+ (void)addStagesFromLevels:(NSArray *)levelNames isCustom:(BOOL)custom
{
	...
	for (int i = 0; i < levelNames.count; i += MaxStageLevelCount)
	{
		...
		int min = (int)MIN(i + MaxStageLevelCount, levelNames.count);
		for (int j = i; j < min; j++)
		{
...
+ (FPMenuButton)touchEndedAtPoint:(CGPoint)point
{
	...
	{
		currentStage++;
		if (currentStage >= (int)[stages count])
			currentStage = (int)[stages count] - 1;
	}
